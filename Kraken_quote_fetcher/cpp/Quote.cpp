...
KrakenBasicConf Quote::getBasicConf() {
    ...
    string path = tool.GetProgramDir(2) + "\\configuration\\fetcher\\basicConfiguration.csv";
    ...
    while (getline(infile, word, ','))
    {
        if (counting % 2 == 0)
        {
            ...
            case 2://用户
                ...
                tool.printOnScreen("登陆用户", word.c_str());
                break;
            case 4://密码
            ...
            case 6://IP
                ...
                tool.printOnScreen("服务器IP", word.c_str());
                break;
            case 8://交易端口
                info.TradePort = atoi(word.c_str());
                tool.printOnScreen("服务器交易端口", word.c_str());
                break;
            case 10://行情端口
                info.QuotePort = atoi(word.c_str());
                tool.printOnScreen("服务器行情端口", word.c_str());
                break;
            case 12://授权码
                strcpy(info.AuthCode, word.c_str());
                tool.printOnScreen("授权码", word.c_str());
                break;
            case 14://程序版本号
                strcpy(info.AppID, word.c_str());
                tool.printOnScreen("程序版本号", word.c_str());
                break;
...
void Quote::DataDeliver(const TapAPIQuoteWhole* info) {
    ...
    if (info)
    {
        ...
        ReleaseMutex(sm_tool.L_Mutex);
        SetEvent(sm_tool.L_Event);
    }
    else {
...
void Quote::Run_()
{
   ...
   go_sharememory sm;
   sm_tool = sm.create();
   deliver_data = (deliver_)MapViewOfFile(sm_tool.MapFile, FILE_MAP_ALL_ACCESS, 0, 0, BUF_SIZE);
   //拉取各个交易所品种编码表
   //QryCommdityInfo();
   //QryContractInfo();
